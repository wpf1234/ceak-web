    ...
    return (
      ...
      <section className="bg-gray-100 px-8 md:px-16 flex flex-col items-center pt-28 pb-56 md:pt-20 md:pb-20 ">
        <div className="max-w-7xl w-full mx-auto ">
          ...
          {/* 底部文字内容 */}
          <div className="text-left text-tc text-lg md:text-2xl max-w-4xl tracking-wider leading-loose font-[family-name:var(--font-sans-light)]">
      ...
      {/* 中间部分 - 循环渲染 */}
      <section className="py-16 px-4 md:px-8 lg:px-16 space-y-24">
        <div className="w-full max-w-7xl mx-auto space-y-24">
          {siteConfig.eduItems.map((section, index) => {
            const isEven = index % 2 === 0; // 偶数项：左文右图；奇数项：左图右文
      ...
      <section className="relative w-full h-[215px] overflow-hidden px-4 md:px-8 lg:px-16">
        <div className="w-full max-w-7xl mx-auto h-full relative rounded-2xl overflow-hidden">
          {/* 背景图片 */}
          <Image
          ...
          <div className="absolute inset-0 bg-black/30 flex items-center rounded-2xl">
            <div className="w-full flex flex-col md:flex-row justify-between items-center md:items-center gap-6 text-white text-center md:text-left px-4">
              {/* 左侧文字 */}
              <div className="text-2xl md:text-4xl whitespace-nowrap font-bold font-[family-name:var(--font-han-sans)] leading-snug">
                我们正在建设明天的教育。
              </div>

              {/* 右侧文字 */}
              <div className="text-base md:text-lg font-[family-name:var(--font-sans-light)] max-w-2xl leading-relaxed">
                CEAK与全球领先的组织合作打造新产品、服务和业务，颠覆教育现在，创造教育未来。
              </div>
            </div>
          </div>
        </div>
