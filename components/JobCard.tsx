...
export const JobCard: React.FC<JobCardProps> = ({ job, index }) => {
  ...
  return (
    ...
    >
      ...
      <p className="text-xl md:text-3xl mb-6 leading-relaxed
                  md:leading-relaxed font-[family-name:var(--font-sans-light)]">{job.description}</p>
      ...
      <ul className="list-none space-y-2">
        {job.requirements.map((req, i) => (
          <li key={i} className="flex items-start gap-2">
            ...
            <span className="text-xl md:text-3xl leading-relaxed
                  md:leading-relaxed font-[family-name:var(--font-sans-light)]">{req}</span>
          </li>
        ))}
      </ul>
    </motion.div>
